...
def test_health():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                "Make sure that you have updated the context length to at least 8192 tokens"
            )
            return True
        else:
        ...
        print(
            "💡 Make sure the service is running: uv run uvicorn api.app:app --reload"
        )
        return False
...
def upload_and_process_file(file_path: str) -> dict:
    ...
    with open(file_path, "rb") as f:
        ...
        response = requests.post(
            endpoint, files=files, data=extra_params, timeout=TIMEOUT
        )
...
def query_document(query: str, mode: str = "hybrid") -> str:
    ...
    response = requests.post(f"{SERVICE_URL}/query", json=payload, timeout=TIMEOUT)
...
async def main():
    ...
    parser.add_argument(
        "file_path", help="Path to file to process (Excel, Office docs, PDF, etc.)"
    )
    parser.add_argument(
        "--service-url",
        default=SERVICE_URL,
