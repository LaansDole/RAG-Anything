...
def analyze_patient_data():
    ...
    print("\n🔗 Comorbidity Analysis:")
    print(
        f"   Patients with comorbidities: {with_comorbidities} ({(with_comorbidities / len(df)) * 100:.1f}%)"
    )
    print(
        f"   Patients without comorbidities: {no_comorbidities} ({(no_comorbidities / len(df)) * 100:.1f}%)"
    )
...
def analyze_clinical_trials():
    ...
    print("\n👥 Enrollment Analysis:")
    print(
        f"   Average target enrollment: {df['enrollment_target'].mean():.0f} participants"
    )
    print(
        f"   Median target enrollment: {df['enrollment_target'].median():.0f} participants"
    )
...
def cross_dataset_analysis():
    ...
    try:
        datasets["symptoms"] = pd.read_excel(
            os.path.join(datasets_dir, "medical_symptoms_small.xlsx")
        )
        datasets["patients"] = pd.read_excel(
            os.path.join(datasets_dir, "patient_records_small.xlsx")
        )
        datasets["abstracts"] = pd.read_csv(
            os.path.join(datasets_dir, "medical_abstracts_medium.csv"), nrows=1000
        )
        datasets["trials"] = pd.read_csv(
            os.path.join(datasets_dir, "clinical_trials_medium.csv"), nrows=1000
        )
        ...
        for name, df in datasets.items():
            ...
            print(
                f"   {name.title()}: {len(df):,} records ({size_mb:.2f} MB in memory)"
            )
        ...
        diabetes_abstracts = datasets["abstracts"][
            datasets["abstracts"]["keywords"].str.contains(
        ...
        diabetes_trials = datasets["trials"][
            datasets["trials"]["condition"].str.contains(
                "Diabetes", case=False, na=False
            )
        ]
        ...
        diabetes_patients = datasets["patients"][
            datasets["patients"]["primary_condition"].str.contains(
                "Diabetes", case=False, na=False
            )
        ]

        print("   Diabetes research pipeline:")
        ...
        if len(diabetes_patients) > 0:
            ...
            print(
                f"     ✅ Positive treatment outcomes: {len(good_response)}/{len(diabetes_patients)} ({(len(good_response) / len(diabetes_patients)) * 100:.1f}%)"
            )
...
def main():
    ...
    try:
        ...
        print("\n\n✅ Analysis Complete!")
        print("💾 Consider saving these insights for RAG system optimization")

    except Exception as e:
        print(f"\n❌ Analysis error: {e}")
        print(
            "💡 Make sure all datasets are generated first: python service/datasets/generate_datasets.py"
        )
