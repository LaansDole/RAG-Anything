        ...
        data.append(
            {
                ...
                "authors": "Smith J, Johnson A, Brown M, et al.",
        ...
        data.append(
            {
                ...
                "location": random.choice(
                    ["United States", "Europe", "Global", "Asia-Pacific"]
                ),
...
def main():
    ...
    symptoms_df.to_excel(
        os.path.join(datasets_dir, "medical_symptoms_small.xlsx"), index=False
    )
    ...
    patients_df.to_excel(
        os.path.join(datasets_dir, "patient_records_small.xlsx"), index=False
    )
    ...
    abstracts_df.to_csv(
        os.path.join(datasets_dir, "medical_abstracts_medium.csv"), index=False
    )
    ...
    trials_df.to_csv(
        os.path.join(datasets_dir, "clinical_trials_medium.csv"), index=False
    )
    ...
    print("\n📈 Dataset Summary:")
    print(
        f"   Small Excel files: 2 files (~{(len(symptoms_df) + len(patients_df))} total records)"
    )
    print(
        f"   Medium CSV files: 2 files (~{(len(abstracts_df) + len(trials_df))} total records)"
    )
