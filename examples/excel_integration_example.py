...
async def method_2_file_based(excel_file_path: str):
    ...
    if result["success"]:
        print(f"✅ Processed Excel file: {result['total_rows']} rows, {result['chunks_created']} chunks")
        
        # Query the data
        answer = await rag.aquery("What is this data about and what insights can you provide?")
        print(f"💬 Answer: {answer}")
    else:
...
def main():
    ...
    parser.add_argument(
        "--method", choices=["dataframe", "file", "both"], default="dataframe",
    ...
    print(f"🚀 RAGAnything Excel Integration Example")
