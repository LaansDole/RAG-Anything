...
class ExcelDataProcessor:
    ...
    def generate_dataset_summary(self, df: pd.DataFrame) -> str:
        ...
        if self.config.include_column_info:
            ...
            for col in df.columns:
                ...
                summary_parts.append(f"- {col}: {col_type} ({non_null_count} non-null, {null_count} null values)")
                ...
                if df[col].dtype == 'object':
                    ...
                    if unique_count <= 10:
                        ...
                        summary_parts.append(f"  Sample values: {', '.join(map(str, unique_values))}")
                    else:
    ...
    def _dataframe_chunk_to_text(self, chunk_df: pd.DataFrame, start_index: int) -> str:
        ...
        text_parts = []
        text_parts.append(f"Data Records {start_index + 1} to {start_index + len(chunk_df)}:")
    ...
    def _dataframe_chunk_to_structured_text(self, chunk_df: pd.DataFrame, start_index: int) -> str:
...
class ExcelRAGIntegration:
    ...
    def __init__(self, rag_anything_instance, excel_config: ExcelProcessingConfig = None):
        self.rag_anything = rag_anything_instance
        self.excel_processor = ExcelDataProcessor(excel_config)
        self.logger = logger
    
    async def process_excel_file(self, file_path: str, doc_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Inserting Excel data into LightRAG: {len(combined_text)} characters")
    ...
    async def process_dataframe_directly(self, df: pd.DataFrame, doc_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Inserting DataFrame data into LightRAG: {len(combined_text)} characters")
            await self.rag_anything.lightrag.ainsert(combined_text)
