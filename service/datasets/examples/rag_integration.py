...
Common Causes: {row['common_causes']}
    ...
    table_markdown = symptoms_df[['symptom_name', 'category', 'severity', 'urgency_level']].to_markdown(index=False)
...
async def process_medical_csv_demo():
    ...
    print(f"📚 Processing medical_abstracts_medium.csv (sample)...")
...
async def clinical_trials_demo():
    ...
    print(f"🧪 Processing clinical_trials_medium.csv (sample)...")
...
async def main():
    ...
    try:
        ...
        print(f"🔗 Next Steps:")
        print(f"   1. Set up your LLM and embedding model APIs")
        print(f"   2. Configure RAGAnything with your credentials")
        print(f"   3. Run the actual RAG processing pipeline")
        print(f"   4. Build medical AI applications on top of the knowledge base")
        
    except Exception as e:
        print(f"\n❌ Demo error: {e}")
        print(f"💡 This is a demonstration script showing RAG integration patterns.")
