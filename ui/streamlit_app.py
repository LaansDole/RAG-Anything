...
def check_service_health(service_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"status": "error", "message": f"Service returned status {response.status_code}"}
    ...
    try:
        ...
        if file_type in ['.xlsx', '.xls']:
            ...
            data = {
                ...
                'convert_to_text': str(st.session_state.get('excel_convert_to_text', True)).lower(),
                'include_summary': str(st.session_state.get('excel_include_summary', True)).lower(),
        ...
        if response.status_code == 200:
            ...
            try:
                if response.headers.get('content-type', '').startswith('application/json'):
                    ...
                    return {
                        'status': 'error',
                        'message': f"Invalid response format from server"
                    }
...
def _try_multimodal_query(query: str, service_url: str, mode: str,
                         multimodal_content: List[Dict]) -> Dict[str, Any]:
...
def query_service(query: str, service_url: str, mode: str = "hybrid",
                 multimodal_content: List[Dict] = None) -> Dict[str, Any]:
    ...
    if multimodal_content and len(multimodal_content) > 0:
        multimodal_result = _try_multimodal_query(query, service_url, mode, multimodal_content)
...
def render_sidebar():
    ...
    st.session_state.advanced_settings = st.sidebar.checkbox(
        "🔧 Advanced Settings",
        value=st.session_state.advanced_settings
    )
    ...
    if st.session_state.advanced_settings:
        ...
        st.session_state.excel_convert_to_text = st.sidebar.checkbox(
            "Convert to Text",
            value=True,
            help="Convert Excel data to natural language"
        )
        
        st.session_state.excel_include_summary = st.sidebar.checkbox(
            "Include Summary",
            value=True,
            help="Include dataset summary in processing"
        )
...
def render_file_upload():
    ...
    st.markdown('<h2 class="main-header">📁 Document Upload & Processing</h2>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        if uploaded_files:
            ...
            for file in uploaded_files:
                ...
                file_type = next((k for k, v in SUPPORTED_FILE_TYPES.items() if file_ext in v), "Other")
    ...
    if uploaded_files and st.button("🚀 Process Files", type="primary"):
        ...
        for i, file in enumerate(uploaded_files):
            ...
            result = upload_and_process_file(file, st.session_state.service_url, file_ext)
            ...
            with status_container:
                if result['status'] == 'success':
                    ...
                    if file_ext in ['.xlsx', '.xls'] and 'total_rows' in result:
                        st.write(f"   📊 Rows: {result['total_rows']}, Chunks: {result.get('chunks_created', 'N/A')}")
                else:
                    st.error(f"❌ Error processing {file.name}: {result['message']}")
        
        # Update session state
        st.session_state.uploaded_files = processed_files
        st.session_state.file_processing_complete = any(f['processed'] for f in processed_files)
...
def render_query_interface():
    ...
    st.markdown('<h2 class="main-header">💬 RAG Assistant Chatbot</h2>', unsafe_allow_html=True)
    
    if not st.session_state.file_processing_complete:
        st.warning("⚠️ Please upload and process files first before starting a conversation.")
        return
    ...
    if send_button and user_input.strip():
        ...
        with st.spinner("Thinking..."):
            ...
            result = query_service(user_input.strip(), st.session_state.service_url, mode, multimodal)
            
            # Add assistant response to chat
            if result['status'] == 'success':
                assistant_message = {
                    'role': 'assistant',
                    'content': result.get('result', 'I received your question but couldn\'t generate a response.'),
                    'timestamp': time.time(),
                    'status': 'success',
                    'mode': mode,
...
def generate_chat_export() -> str:
    ...
    export_lines = [
        f"RAG Assistant Conversation Export",
    ...
    for message in st.session_state.chat_history:
        ...
        if message['role'] == 'user':
            ...
            mode_info = f" (Mode: {message.get('mode', 'unknown')})" if message.get('mode') else ""
            export_lines.extend([
...
def render_query_history():
    ...
    for i, query_result in enumerate(reversed(st.session_state.query_history)):
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(query_result['timestamp']))
        
        with st.expander(f"Query {len(st.session_state.query_history) - i}: {query_result['query'][:50]}... ({timestamp})"):
...
def render_analytics():
    ...
    if st.session_state.uploaded_files:
        ...
        processed_count = sum(1 for f in st.session_state.uploaded_files if f['processed'])
        ...
        for file_info in st.session_state.uploaded_files:
            file_data.append({
                ...
                "Details": file_info['result'].get('message', '') if not file_info['processed'] else "Success"
            })
    ...
    if st.session_state.query_history:
        ...
        successful_queries = sum(1 for q in st.session_state.query_history if q['result']['status'] == 'success')
        multimodal_queries = sum(1 for q in st.session_state.query_history if q['has_multimodal'])
...
def main():
    ...
    st.markdown("A comprehensive interface for document processing and intelligent querying")
    
    # Tab navigation
    tab1, tab2, tab3, tab4 = st.tabs(["📁 Upload", "🔍 Query", "📚 History", "📈 Analytics"])
